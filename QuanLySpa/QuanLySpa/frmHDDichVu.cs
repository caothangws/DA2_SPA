...
namespace QuanLySpa
{
    public partial class frmHDDichVu : Form
    {
        ...
        int dong = 0;
        int dong_dv = 0;
        private string manv; // bien luu manv
        public frmHDDichVu(string maNV)
        {
            InitializeComponent();
            manv = maNV; // luu manv vao bien
        }
        void set_null()
        {
            cbxTenKH.Text = "";
        ...
        {
            //dich vu
            string dichvu = string.Format("SELECT * FROM DICHVU");
            adp = new SqlDataAdapter(dichvu, conn);
            DataTable dt_dichvu = new DataTable();
            adp.Fill(dt_dichvu);
            col_TENDV.DataSource = dt_dichvu;
            col_TENDV.DisplayMember = "TENDV";
            col_TENDV.ValueMember = "MADV";
        }

        void load_nhanviendv()
        {
            ...
            if (cbxTenKH.SelectedValue != null)
            {
                try
                {
                    ...
                    if (dtKH.Rows.Count > 0)
                    {
                        ...
                        // Làm mới bảng dịch vụ
                        foreach (DataGridViewRow row in bangDV.Rows)
                        {
                            if (row.Cells[1].Value != null) // Nếu có  dịch v được chọn
                            {
                                double soluong = double.Parse(row.Cells[2].Value.ToString());
                                double giaban = double.Parse(row.Cells[4].Value.ToString());
                                row.Cells[5].Value = (soluong * giaban * (1 - uudai / 100.0)).ToString(); // Tính lại thành tiền
                        ...
                        for (int i = 0; i < bangDV.Rows.Count - 1; i++)
                        {
                            if (bangDV.Rows[i].Cells[5].Value != null)
                                tongtien = tongtien + int.Parse(bangDV.Rows[i].Cells[5].Value.ToString());
        ...
        private void bangDV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dong_dv != -1)
            {
                if (bangDV.Rows[dong_dv].Cells[1].Value != null)
                {
                    col_SoLuongDV.ReadOnly = false;
                    col_NhanVien.ReadOnly = false;
                }
                else
                {
                    col_SoLuongDV.ReadOnly = true;
                    col_NhanVien.ReadOnly = true;
                }
            }
        }

        private void bangDV_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0 || e.ColumnIndex < 0) return;
            int uudai = 0;
            try
            {
                ...
                dong_dv = e.RowIndex;
                //tinh tong tien
                if (bangDV.Rows[dong_dv].Cells[3].Value != null && bangDV.Rows[dong_dv].Cells[4].Value != null)
                {
                    double soluong = double.Parse(bangDV.Rows[dong_dv].Cells[3].Value.ToString());
                    double giatien = double.Parse(bangDV.Rows[dong_dv].Cells[4].Value.ToString());
                    bangDV.Rows[dong_dv].Cells[5].Value = (soluong * giatien * (1 - uudai / 100.0)).ToString();
                }
                //lay thong tin dv
                string madv = bangDV.Rows[e.RowIndex].Cells[1].Value?.ToString();
                if (!string.IsNullOrEmpty(madv))
                {
                    string str_dv = string.Format("SELECT * FROM DICHVU WHERE MADV = '" + madv + "'");
                    SqlDataAdapter adp = new SqlDataAdapter(str_dv, conn);
                    DataTable dt_lt = new DataTable();
                    adp.Fill(dt_lt);
                    if (dt_lt.Rows.Count > 0)
                    {
                        bangDV.Rows[dong_dv].Cells[0].Value = dt_lt.Rows[0]["MADV"].ToString();
                        bangDV.Rows[dong_dv].Cells[2].Value = dt_lt.Rows[0]["THOIGIANDT"].ToString();
                        bangDV.Rows[dong_dv].Cells[4].Value = dt_lt.Rows[0]["GIATIEN"].ToString();
                    }
                }
                //them so luong con trong
                for (int i = 0; i < bangDV.Rows.Count - 1; i++)
                {
                    if (bangDV.Rows[i].Cells[1].Value != null && bangDV.Rows[i].Cells[3].Value == null)
                        bangDV.Rows[i].Cells[3].Value = "1";
                }
        ...
        private void bangDV_RowsRemoved(object sender, DataGridViewRowsRemovedEventArgs e)
        {
            tongtien = 0;
            for (int i = 0; i < bangSP.Rows.Count - 1; i++)
            {
        ...
        private void lbXoaDV_Click(object sender, EventArgs e)
        {
            try
            {
        ...
        private bool KTThongTin()
        {
            ...
            for (int i = 0; i < bangDV.Rows.Count - 1; i++)
            {
                ...
                if (bangDV.Rows[i].Cells[6].Value == null)
                {
                    MessageBox.Show("Vui lòng chọn nhân viên.");
                    return false;
                }
            }
            for (int i = 0; i < bangDV.Rows.Count - 2; i++)
            {
                for (int j = i + 1; j < bangDV.Rows.Count - 1; j++)
                {
                    if (bangDV.Rows[i].Cells[0].Value.ToString() == bangDV.Rows[j].Cells[0].Value.ToString())
                    {
                        MessageBox.Show("Trùng dịch vụ" + bangDV.Rows[i].Cells[0].Value.ToString());
                        return false;
                    }
                }
            }
        ...
        private void ThemDLCTDV()
        {
            //them du lieu bang ctdv
            for (int i = 0; i < bangDV.Rows.Count - 1; i++)
            {
                string ctdv = string.Format("INSERT INTO CTHDDV (SOHD,MADV,MANV,SOLUONG,THANHTIEN)VALUES (@SOHD,@MADV,@MANV,@SOLUONG,@THANHTIEN)");
                cmd = new SqlCommand(ctdv, conn);
                cmd.Parameters.AddWithValue("@SOHD", txtMaHD.Text);
                cmd.Parameters.AddWithValue("@MADV", bangDV.Rows[i].Cells[0].Value.ToString());
                cmd.Parameters.AddWithValue("@MANV", bangDV.Rows[i].Cells[6].Value.ToString());
                cmd.Parameters.AddWithValue("@SOLUONG", bangDV.Rows[i].Cells[3].Value.ToString());
                cmd.Parameters.AddWithValue("@THANHTIEN", bangDV.Rows[i].Cells[5].Value.ToString());
                cmd.ExecuteNonQuery();
            }
        }
        ...
        private void CapNhatDiemKH()
        {
            ...
            string diemtl = @"
                ...
                + ( SELECT COALESCE(SUM(DICHVU.DIEMDV  * CTHDDV.SOLUONG), 0)
                FROM CTHDDV
                INNER JOIN DICHVU ON CTHDDV.MADV = DICHVU.MADV
                WHERE CTHDDV.SOHD = @SOHD)
        ...
        private void btnXemHD_Click(object sender, EventArgs e)
        {
            ...
            JOIN CTHDDV ON HOADON.SOHD = CTHDDV.SOHD
