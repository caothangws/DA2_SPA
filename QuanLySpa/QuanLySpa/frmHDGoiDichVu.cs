...
namespace QuanLySpa
{
    public partial class frmHDGoiDichVu : Form
    {
        ...
        int dong_gdv = 0;
        private string manv;
        public frmHDGoiDichVu(string maNV)
        {
            InitializeComponent();
            manv = maNV;
        }
        ...
        private void frmHDGoiDichVu_Load(object sender, EventArgs e)
        {
            set_null();
            load_kh();
            load_txtNV();
        ...
        private void load_goidichvu()
        {
            //dich vu
            string dichvu = string.Format("SELECT * FROM GOIDICHVU");
            adp = new SqlDataAdapter(dichvu, conn);
            DataTable dt_dichvu = new DataTable();
            adp.Fill(dt_dichvu);
            col_TENGDV.DataSource = dt_dichvu;
            col_TENGDV.DisplayMember = "TENGOI";
            col_TENGDV.ValueMember = "MAGOIDV";
        }
        ...
        private void bangSP_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < bangGDV.Rows.Count - 1; i++)
                {
                    if (bangGDV.Rows[i].Cells[5].Value != null)
                        tongtien = tongtien + int.Parse(bangGDV.Rows[i].Cells[5].Value.ToString());
                }
        ...
        private void bangGDV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            dong_gdv = e.RowIndex;
            if (dong_gdv != -1)
            {
                if (bangGDV.Rows[dong_gdv].Cells[1].Value != null)
                {
                    col_SoLuongGDV.ReadOnly = false;
                    col_NhanVien.ReadOnly = false;
                }
                else
                {
                    col_SoLuongGDV.ReadOnly = true;
                    col_NhanVien.ReadOnly = true;
                }
            }
        }

        private void bangGDV_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0 || e.ColumnIndex < 0) return;
            int uudai = 0;
            try
            {
                ...
                dong_gdv = e.RowIndex;
                //tinh tong tien
                if (bangGDV.Rows[dong_gdv].Cells[3].Value != null && bangGDV.Rows[dong_gdv].Cells[4].Value != null)
                {
                    double soluong = double.Parse(bangGDV.Rows[dong_gdv].Cells[3].Value.ToString());
                    double giatien = double.Parse(bangGDV.Rows[dong_gdv].Cells[4].Value.ToString());
                    bangGDV.Rows[dong_gdv].Cells[5].Value = (soluong * giatien * (1 - uudai / 100.0)).ToString();
                }
                //lay thong tin dv
                string magdv = bangGDV.Rows[e.RowIndex].Cells[1].Value?.ToString();
                if (!string.IsNullOrEmpty(magdv))
                {
                    string str_dv = string.Format("SELECT * FROM GOIDICHVU WHERE MAGOIDV = '" + magdv + "'");
                    SqlDataAdapter adp = new SqlDataAdapter(str_dv, conn);
                    DataTable dt_lt = new DataTable();
                    adp.Fill(dt_lt);
                    if (dt_lt.Rows.Count > 0)
                    {
                        bangGDV.Rows[dong_gdv].Cells[0].Value = dt_lt.Rows[0]["MAGOIDV"].ToString();
                        bangGDV.Rows[dong_gdv].Cells[2].Value = dt_lt.Rows[0]["THOIGIANDT"].ToString();
                        bangGDV.Rows[dong_gdv].Cells[4].Value = dt_lt.Rows[0]["GIATIEN"].ToString();
                    }
                }
                //them so luong con trong
                for (int i = 0; i < bangGDV.Rows.Count - 1; i++)
                {
                    if (bangGDV.Rows[i].Cells[1].Value != null && bangGDV.Rows[i].Cells[3].Value == null)
                        bangGDV.Rows[i].Cells[3].Value = "1";
                }
        ...
        private void lbXoaGDV_Click(object sender, EventArgs e)
        {
            try
            {
                bangGDV.Rows.RemoveAt(dong_gdv);
            }
            catch
            {
        ...
        private bool KTThongTin()
        {
            ...
            for (int i = 0; i < bangGDV.Rows.Count - 1; i++)
            {
                ...
                if (bangGDV.Rows[i].Cells[6].Value == null)
                {
                    MessageBox.Show("Vui lòng chọn nhân viên.");
                    return false;
                }
            }
            for (int i = 0; i < bangGDV.Rows.Count - 2; i++)
            {
                for (int j = i + 1; j < bangGDV.Rows.Count - 1; j++)
                {
                    if (bangGDV.Rows[i].Cells[0].Value.ToString() == bangGDV.Rows[j].Cells[0].Value.ToString())
                    {
                        MessageBox.Show("Trùng gói dịch vụ" + bangGDV.Rows[i].Cells[0].Value.ToString());
                        return false;
                    }
                }
            }
        ...
        private void ThemDLCTGDV()
        {
            //them du lieu bang ctgdv
            for (int i = 0; i < bangGDV.Rows.Count - 1; i++)
            {
                string ctdv = string.Format("INSERT INTO CTHDGOI (SOHD,MAGOIDV,MANV,SOLUONG,THANHTIEN)VALUES (@SOHD,@MAGOIDV,@MANV,@SOLUONG,@THANHTIEN)");
                cmd = new SqlCommand(ctdv, conn);
                cmd.Parameters.AddWithValue("@SOHD", txtMaHD.Text);
                cmd.Parameters.AddWithValue("@MAGOIDV", bangGDV.Rows[i].Cells[0].Value.ToString());
                cmd.Parameters.AddWithValue("@MANV", bangGDV.Rows[i].Cells[6].Value.ToString());
                cmd.Parameters.AddWithValue("@SOLUONG", bangGDV.Rows[i].Cells[3].Value.ToString());
                cmd.Parameters.AddWithValue("@THANHTIEN", bangGDV.Rows[i].Cells[5].Value.ToString());
                cmd.ExecuteNonQuery();
            }
        }
        ...
        private void CapNhatDiemKH()
        {
          ...
          WHERE CTHD.SOHD = @SOHD)
           + ( SELECT COALESCE(SUM(GOIDICHVU.DIEMGDV * CTHDGOI.SOLUONG), 0)
          FROM CTHDGOI
          INNER JOIN GOIDICHVU ON CTHDGOI.MAGOIDV = GOIDICHVU.MAGOIDV
          WHERE CTHDGOI.SOHD = @SOHD)
        ...
        private void bangGDV_RowsRemoved(object sender, DataGridViewRowsRemovedEventArgs e)
        {
            tongtien = 0;
            for (int i = 0; i < bangSP.Rows.Count - 1; i++)
            {
            ...
            if (cbxTenKH.SelectedValue != null)
            {
                try
                {
                    ...
                    if (dtKH.Rows.Count > 0)
                    {
                        ...
                        // Làm mới bảng gói dịch vụ
                        foreach (DataGridViewRow row in bangGDV.Rows)
                        {
                            if (row.Cells[1].Value != null) // Nếu có gói dịch v được chọn
                            {
                                double soluong = double.Parse(row.Cells[2].Value.ToString());
                                double giaban = double.Parse(row.Cells[4].Value.ToString());
                                row.Cells[5].Value = (soluong * giaban * (1 - uudai / 100.0)).ToString(); // Tính lại thành tiền
        ...
        {
            string str_hd = @"SELECT HOADON.SOHD FROM HOADON
            JOIN CTHDGOI ON HOADON.SOHD = CTHDGOI.SOHD
            LEFT JOIN CTHD  ON HOADON.SOHD =CTHD.SOHD
            WHERE  not exists ( select 1 from CTHDLT join LIEUTRINH on LIEUTRINH.MALT = CTHDLT.MALT where CTHDLT.SOHD = HOADON.SOHD)
